...
export async function DELETE(
  ...
  try {
    const user = await getSessionUser();
    const { id } = await params;
    const record = await prisma.ocrRecord.findFirst({ where: { id, companyId: user.companyId } });
    if (!record) return NextResponse.json({ error: { code: "NOT_FOUND", message: "見つかりません" } }, { status: 404 });
